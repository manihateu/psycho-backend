...
export class RecomendationService {
  ...
  private async getInteractionMatrix(): Promise<
    Array<{ userId: number; courseId: number; score: number }>
  > {
  ...
  async recommendCourses(userId: number, limit: number = 5): Promise<any[]> {
    ...
    const recommendedCoursesIds = Array.from(recommendedCourses).slice(
      0,
      limit,
    );
    return await this.prisma.course.findMany({
  ...
  async addInteraction(
    userId: number,
    courseId: number,
    interactionType: 'LIKE' | 'STARTED' | 'COMPLETED',
  ) {
